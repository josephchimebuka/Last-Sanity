...
const Dropdown2: React.FC<DropdownProps> = ({ title, items, openDropdown, setOpenDropdown, index,notification }) => {
  ...
  <li className="flex justify-between listtags md:py-2 py-2 px-2 md:px-2 uppercase" key={item.id}>
    ...
    {item.price && <p className="text-[15px] lg:text-[16px] ml-3 text-[#FE9346]">N{item.price}</p>}
         ...
         {notification && (
                <p className='text-[8px] lg:text-[12px] italics md:py-2 py-2 px-2 md:px-2 text-[#FE9346]'>{notification}</p>
              )}
        </ul>
      )}
    </div>
